...
def createDataSet():
    ...
    #         ['no'],
    #         ['no']]
    # labels  露出水面、脚蹼，注意：这里的labels是写的 dataSet中特征的含义，并不是对应的分类标签或者说目标变量
...
def calcShannonEnt(dataSet):
    ...
    for featVec in dataSet:
        # 将当前实例的标签存储，即每一行数据的最后一个数据代表的是标签，-1表示取最后一列
        # currentLabel是yes/no
...
def splitDataSet(dataSet, index, value):
    ...
    Desc：
        ...
        就是依据index列进行分类，如果index列的数据等于 value的时候，就要将 index划分到我们创建的新的数据集中
    Args:
        dataSet  -- 数据集                 待划分的数据集
...
def chooseBestFeatureToSplit(dataSet):
    ...
    # 答：因为我们在根据一个特征计算香农熵的时候，该特征的分类值是相同，这个特征这个分类的香农熵为0
...
def ContactLensesTest():
    ...
    # 加载隐形眼镜相关的 文本文件 数据
    fr = open('/Users/yinxing/03workspace/self/MLAction/src/ML/datasets/3.DecisionTree/lenses.txt')
    # 解析数据，获得 features 数据,解析tab键分隔的数据行
