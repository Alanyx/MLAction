...
def load_data_set():
    ...
    data_arr = []
    label_arr = []
    f = open('/Users/yinxing/03workspace/self/MLAction/src/ML/datasets/5.Logistic/TestSet.txt', 'r')
...
def grad_ascent(data_arr, class_labels):
    ...
    :return: 
    """
    # data_arr:因为传进来的是一个数组，用这个比较不容易搞混
    ...
    for k in range(max_cycles):
        ...
        # label_mat=0或者1，error分别对应到公式中的log括号中的项: -y*log(y_hat) -(1-y)*log(1-y_hat)
        error = label_mat - h
...
def plot_best_fit(weights):
    ...
    for i in range(n):
        if int(label_mat[i]) == 1:
            x_cord1.append(data_arr[i, 1])  ## todo 为什么[i,2]?
    ...
    x0最开始就设置为1了， x2就是我们画图的y值，而f(x)被我们磨合误差给算到w0,w1,w2身上去了
...
def stoc_grad_ascent0(data_mat, class_labels):
    ...
    :return: 得到的最佳回归系数
    """
    m, n = np.shape(data_mat)  ## todo 为什么除去最后一列？
    alpha = 0.01
    weights = np.ones(n)
    for i in range(m):
        ...
        weights = weights + alpha * error * data_mat[i]  ## todo data_mat[i]具体的一项形状
    return weights
...
def colic_test():
    ...
    其实最主要的的部分，比如缺失值的补充（真的需要学会的），人家已经做了
    :return: error_rate--分类错误率
    """
    f_train = open('/Users/yinxing/03workspace/self/MLAction/src/ML/datasets/5.Logistic/HorseColicTraining.txt', 'r')
    f_test = open('/Users/yinxing/03workspace/self/MLAction/src/ML/datasets/5.Logistic/HorseColicTest.txt', 'r')
    training_set = []
    training_labels = []
